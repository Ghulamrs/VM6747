...
# (`shci --target=tms6747 -c`, which runs asm6x on what it wrote), and that
...
for case in tests/cases/*.shm tests/load/*.shm; do
    ...
    if ! ( ulimit -t 20; SHC_AS="$ASM6X" "$SHC" --target=tms6747 -nologo -c "$case" -o "$OUT/$name.obj" < /dev/null ) > "$OUT/$name.err" 2>&1; then
