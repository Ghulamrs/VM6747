...
rem  compiled with `shci --target=tms6747`, which assembles what it wrote and
...
rem  <case>.out. The linker is the judge. shci.exe, asm6x.exe and the runtime
...
set ROOT=%~1
set BIN=%~2
set SHC_TI=C:\ti\ccsv7\tools\compiler\ti-cgt-c6000_8.2.2
set SHC_TILIB=C:\Users\GRA\Documents\VM6747\tilib
if not exist %BIN%\shci.exe (echo ti-link.cmd: no shci.exe in %BIN% & exit /b 1)
...
if not exist %SHC_TI%\bin\lnk6x.exe (echo ti-link.cmd: no lnk6x under %SHC_TI% & exit /b 1)
if not exist %SHC_TILIB%\rts6740_elf_eh.lib (echo ti-link.cmd: no rts6740_elf_eh.lib - see Emulator/tests/ti.sh & exit /b 1)
...
for %%f in (%ROOT%\tests\cases\*.shm %ROOT%\tests\load\*.shm) do (
    ...
    %BIN%\shci.exe --target=tms6747 -nologo -S %%f -o %OUT%\!NAME!.s > nul 2>&1
    if errorlevel 1 (set /a refused+=1) else if defined KNOWN (
        %BIN%\shci.exe --target=tms6747 -nologo %%f -o %OUT%\!NAME!.out > %OUT%\!NAME!.log 2>&1
